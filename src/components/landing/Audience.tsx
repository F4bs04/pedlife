          ...
          <div className="gap-5 flex max-md:flex-col max-md:items-stretch">
            {audiences.map((audience, index) => (
              <div key={index} className="w-[33%] max-md:w-full max-md:ml-0">
                <div className="bg-white shadow-[0px_4px_6px_rgba(0,0,0,0.1)] flex grow flex-col items-stretch text-center w-full p-8 rounded-xl max-md:mt-[37px] max-md:px-5">
                  <img
                    src={audience.icon}
                    alt={audience.title}
                    className="aspect-[1] object-contain w-16 self-center rounded-full"
                  />
                  <h3 className="bg-[rgba(0,0,0,0)] text-xl text-gray-800 font-semibold whitespace-nowrap mt-6 pb-3 px-[70px] max-md:px-5">
                    {audience.title}
                  </h3>
                  <div className="bg-[rgba(0,0,0,0)] flex flex-col items-stretch text-base text-gray-600 font-normal mt-4 pb-3 px-2.5">
